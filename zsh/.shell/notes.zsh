...
change_vault () {
	dir=$(ls -d $NOTES_DIR/* | fzf --preview='ls -al {}' --bind shift-up:preview-page-up,shift-down:preview-page-down)
	export NOTES_DIR=$dir
}
...
notes () {
	...
	do
		case $opt in
			(c) file_path=$(ls -d $NOTES_DIR/* | fzf --tac --preview='less {}' --bind shift-up:preview-page-up,shift-down:preview-page-down)
				if [[ -z "$file_path" ]]
				then
					return 1
				fi ;;
	...
	then
		...
		do
			...
			unset response
			cat $headerline_file | gsed "${headerline_num}s/ @remind//" > /tmp/tmp
...
cont_note () {
	...
	do
		case $opt in
			(c) date=$(ls -d $NOTES_DIR/* | fzf --preview='less {}' --bind shift-up:preview-page-up,shift-down:preview-page-down | cut -d "/" -f 5 | cut -d "." -f 1)  ;;
			(d) date="$OPTARG"  ;;
			(\?) echo "Invalid option: -$OPTARG" >&2 ;;
		esac
	done
	...
	if [[ "$from_file_path" != "$NOTES_DIR" && ! -f "$from_file_path" ]]
	then
